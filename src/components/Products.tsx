...
export default function Products() {
  ...
  return (
    <section id="products" className="py-24 bg-gray-900 relative overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative">
            ...
            <button
              ...
              className={`px-5 py-2 rounded-full text-sm font-medium transition-all duration-300 capitalize magnetic-btn ${
                filter === cat
                  ...
                  : 'bg-white/5 text-gray-400 hover:bg-white/10 hover:text-white border border-white/10 hover:border-blue-500/30'
              }`}
            >
              {cat}
            </button>
        ...
        >
          {filtered.map((project) => (
            <TiltCard
              key={project.id}
              onClick={() => project.liveUrl ? window.location.href = project.liveUrl : setSelectedProject(project)}
              className="group cursor-pointer rounded-2xl bg-white/[0.03] border border-white/10 overflow-hidden hover:border-blue-500/30 hover:bg-white/[0.06] transition-all duration-300 hover-lift border-glow"
              intensity={6}
            >
              <div className="relative h-48 overflow-hidden">
                ...
                <div className="absolute bottom-3 left-3">
                  <span className="px-3 py-1 rounded-full bg-white/10 backdrop-blur-sm text-white text-xs border border-white/20">
                    {project.category}
                  </span>
                </div>
                {/* View overlay icon */}
              ...
              <div className="p-6">
                ...
                <div className="flex flex-wrap gap-1.5">
                  {project.technologies.slice(0, 3).map((tech) => (
                    <span key={tech} className="px-2 py-1 bg-white/5 text-gray-300 text-xs rounded-md border border-white/10 hover:border-blue-500/30 hover:text-blue-400 transition-all duration-200">
                      {tech}
                    </span>
                  ))}
                  {project.technologies.length > 3 && (
                    <span className="px-2 py-1 bg-white/5 text-gray-500 text-xs rounded-md border border-white/10">
                      +{project.technologies.length - 3}
                    </span>
                  )}
                </div>
      ...
      {selectedProject && (
        ...
        >
          <div
            className={`bg-gray-900 border border-gray-700 rounded-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto shadow-2xl shadow-blue-500/10 transition-all duration-300 ${
              animatingOut
